#!/usr/bin/env python3
"""
BitLut patch v2: declare the TotalCaloriesBurnedRecord Health Connect
permission in AndroidManifest.xml, fixing the permission dialog not
appearing at all.

Supersedes patch_manifest_total_calories_permission_v1.py, which failed the
real :app:assembleDebug build with:

    Execution failed for task ':app:processDebugMainManifest'.
    > com.android.manifmerger.ManifestMerger2$MergeFailureException:
      Error parsing .../AndroidManifest.xml
    Caused by: org.xml.sax.SAXParseException; lineNumber: 51;
      columnNumber: 66; The string "--" is not permitted within comments.

v1's diagnosis (the permission dialog never appearing because
TotalCaloriesBurnedRecord was requested at runtime but never declared in
the manifest) was correct and is unchanged here -- see below. What was
wrong in v1 was the new XML comment's own wording: it used a literal "--"
as a prose dash ("...permission dialog for it -- omitting it..."), which
is illegal inside an XML comment body per the XML spec, regardless of
context or intent. v1's own compile gate caught this and died before
committing -- but apply_insertion had already written the broken comment
to disk, so it may still be sitting there uncommitted. This version detects
that specific broken text and repairs just the one bad substring in place,
then behaves identically to v1 for a clean, unpatched starting state.
...
Usage:
    python3 patch_manifest_total_calories_permission_v2.py
"""
...
BACKUP_DIR = ROOT / ".bitlut_patch_backup" / "manifest_total_calories_permission_v2"
...
def main() -> None:
    ...
    if broken_fragment in manifest_text:
        # patch_manifest_total_calories_permission_v1.py's insertion used a
        # literal "--" inside an XML comment body, which is illegal per the
        # XML spec (SAXParseException: "The string \"--\" is not permitted
        # within comments."). That version's own compile gate caught this
        # and died before committing, but apply_insertion had already
        # written the broken text to disk -- so it may still be sitting
        # there, uncommitted, exactly as it was before that failure. Repair
        # just the one bad substring in place rather than re-inserting the
        # whole block, so this is safe to run regardless of whether that
        # broken text is present.
        print("  repairing invalid XML comment left by a prior failed patch attempt")
        backup_once(manifest_path)
        write(manifest_path, manifest_text.replace(broken_fragment, fixed_fragment, 1))
        print(f"  applied: {manifest_path.name} (repaired -- to ; inside comment)")
    else:
        apply_insertion(
            manifest_path,
            anchor=(
    ...
    commit = subprocess.run(
        [
            ...
            "manifest (was blocking the permission dialog entirely); fix "
            "invalid XML comment (-- not allowed in comment body); document "
            "the calorie-estimate exception in the sprint contract",
        ],
        cwd=ROOT,
    )
