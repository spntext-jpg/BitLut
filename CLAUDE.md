...
- Daily Huawei Steps use stable per-day `clientRecordVersion` values and no longer perform a recurring 7-day time-range delete. Never restore that delete: once workout-scoped Steps existed, it created repeated downstream deletion/upsertion churn. Health Connect binder/IPC write failures abort the remaining provider calls and defer retry to WorkManager backoff rather than rapid inline replay.
