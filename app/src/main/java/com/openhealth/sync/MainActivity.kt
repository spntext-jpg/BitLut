...
class MainActivity : ComponentActivity() {
    ...
    private val requiredHealthConnectPermissions: Set<String> = setOf(
    ...
    private val googlePermissionLauncher = registerForActivityResult(
        ...
        AppLogger.i("MainActivity", "Returned from Health Connect permission request")
    ...
    private fun requestGooglePermissionsOrOpenProvider() {
        ...
        if (status == HealthConnectClient.SDK_AVAILABLE) {
            AppLogger.i("MainActivity", "Opening Health Connect permission screen")
            googlePermissionLauncher.launch(requiredHealthConnectPermissions)
            return
        }

        AppLogger.w("MainActivity", "Health Connect is not available, status=$status")
        Toast.makeText(
            this,
            "Health Connect is required. Opening install page.",
            Toast.LENGTH_LONG
        ).show()
    ...
    private fun requestHuaweiAuthorizationOrInstallHms() {
        if (!HmsCoreHelper.isInstalled(this)) {
            AppLogger.w("MainActivity", "HMS Core is missing; opening install page")
            Toast.makeText(
                this,
        ...
        if (!HuaweiConfig.hasDeveloperAppId()) {
            Toast.makeText(
                this,
                "Huawei App ID is not configured.",
                Toast.LENGTH_LONG
            ).show()
            return
        }
        ...
        runCatching {
            ...
            Toast.makeText(
                this,
                "Не удалось открыть авторизацию Huawei Health",
                Toast.LENGTH_LONG
            ).show()
        }
    }
...
fun MainExpressiveLayout(
    ...
    Scaffold(
        topBar = {
            TopAppBar(
                title = {
                    Text("BitLut Health", fontWeight = FontWeight.Black)
                }
            )
        },
        ...
        Column(
            ...
            Card(
                ...
                Column(Modifier.padding(24.dp)) {
                    ...
                    Text(
                        "Последняя успешная: ${uiState.lastSyncTime}",
            ...
            Button(
                ...
                enabled = !uiState.isSyncing &&
                    uiState.hasGooglePermissions &&
                    uiState.isHuaweiAuthorized,
                modifier = Modifier.fillMaxWidth()
            ) {
